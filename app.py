...
st.markdown("""
    <style>
    .main-title { font-weight: 400 !important; font-size: 46px !important; margin: 0; padding: 0; }
    .sub-title { font-weight: 300 !important; font-size: 20px !important; margin: 0; padding: 0; color: #aaaaaa !important; }
...
t_start_bt = st.sidebar.number_input("トレイリング開始 (%)", 0.1, 5.0, 0.5, step=0.05)/100
t_pct_bt = st.sidebar.number_input("下がったら成行注文 (%)", 0.1, 5.0, 0.2, step=0.05)/100
s_loss_bt = st.sidebar.number_input("損切り (%)", -5.0, -0.1, -0.7, step=0.05)/100

# --- 6. メインレイアウト ---
st.markdown(f"""<div style='margin-bottom: 20px;'><h1 class='main-title'>FORE CASTER</h1><h3 class='sub-title'>SCREENING & BACKTEST | ver 1.64</h3></div>""", unsafe_allow_html=True)
...
with tab_top:
    ...
    with st.expander(f"リアルタイム指標 ({now_jst})", expanded=True):
        ...
        for name, info in m_data.items():
            if info["val"]:
                ...
                cards_html += f'<div class="metric-card"><div class="card-label">{name}</div><div class="card-value">{v}</div><div class="delta-badge {cls}">{"＋" if info["pct"]>=0 else ""}{info["pct"]:.2f}%</div></div>'
    ...
    with st.expander("🔍 スキャン条件設定", expanded=False):
        s_days = st.slider("分析期間 (過去日数)", 5, 30, 20); s_start = st.time_input("エントリ開始", time(9, 0)); s_end = st.time_input("エントリ終了", time(9, 30)); s_vwap = st.checkbox("VWAP条件必須", value=True)
...
with tab_bt:
    ...
    if st.button("バックテスト実行", type="primary", use_container_width=True):
        trades = []; progress = st.progress(0); s_bt = datetime.now() - timedelta(days=days_back_bt)
                    ...
                    for ts, row in day.iterrows():
                        if not in_pos:
                            if start_entry_bt <= ts.time() <= end_entry_bt and gap_min_bt <= gap <= gap_max_bt:
                                if (not use_vwap_bt or row['Close'] > row['VWAP']) and (not use_ema_bt or row['Close'] > row['EMA5']) and (not use_rsi_bt or (row['RSI']>45 and row['RSI']>row['RSI_P'])) and (not use_macd_bt or row['MH']>row['MHP']):
                                    entry_p = row['Close'] * 1.0003; in_pos = True; stop_p = entry_p * (1 + s_loss_bt); t_high = row['High']
                        else:
                            t_high = max(t_high, row['High'])
                            if not t_active and t_high >= entry_p * (1 + t_start_bt): t_active = True
                            ex_p = None
                            if t_active and row['Low'] <= t_high * (1 - t_pct_bt): ex_p = t_high * (1 - t_pct_bt) * 0.9997
