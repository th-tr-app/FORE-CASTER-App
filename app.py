...
# --- 6. メインレイアウト ---
st.markdown(f"<div style='margin-bottom: 20px;'><h1 class='main-title'>FORE CASTER</h1><h3 class='sub-title'>SCREENING & BACKTEST | ver 1.65</h3></div>", unsafe_allow_html=True)
    ...
    if st.button("バックテスト実行", type="primary", use_container_width=True):
        ...
        if trades:
            ...
            for tk in t_list:
                ...
                rpt.append(f"トレード数: {len(tdf)} | 勝率: {(tdf['PnL']>0).mean():.1%} | 利益平均: {avg_w:+.2%} | 損失平均: {avg_l:+.2%} | PF: {tw.sum()/abs(tl.sum()) if not tl.empty and tl.sum()!=0 else 0:.2f} | 期待値: {tdf['PnL'].mean():+.2%}\n")
