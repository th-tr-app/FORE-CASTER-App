...
# 移動平均・EMAの共通選択肢
MA_CRITERIA = [
    "5MA ＞ 10MA ＞ 25MA (5MAが25MAを上抜けた直後)",
    "5MA ≒ 10MA（収束 / 5MAが10MAを上抜けた直後）",
    "5営業日以内に5MA or 10MAを上抜け (25MA横ばい/緩やか)"
]
EMA_CRITERIA = [
    "EMA9 ＞ EMA21 ＞ EMA50 (価格はEMA9～21の上)",
    "EMA9 ≒ EMA21 (価格はEMA21付近 or やや上)",
    "EMA9 と EMA21 が収束・クロス (価格はEMA9～21の間)"
]
...
DEFAULT_CONFIG = {
    "NORMAL": {
        ...
        "mcap": 500, "price": (500, 5000), "ma25": (0.0, 7.0), "vol": 10, "cross": MA_CRITERIA[0], 
        "margin": (3.0, 10.0), "per": (10.0, 30.0), "ema": EMA_CRITERIA[0], "adx": (25, 40), 
    ...
    "DEFENSIVE": {
        ...
        "mcap": 2000, "price": (500, 5000), "ma25": (-3.0, 2.0), "vol": 20, "cross": MA_CRITERIA[1], 
        "margin": (1.5, 3.0), "per": (10.0, 20.0), "ema": EMA_CRITERIA[1], "adx": (10, 20), 
        "atr": (1.0, 2.0), "rci": (-20, 30), "rsi": (40, 55), "bb": (-1.0, 0.0), "rate": 4, "vol_up": 1.1
    },
    "RANGE": {
        "sector": ["商社", "石油・ガス", "銀行", "電機", "半導体周辺", "建設", "プラント"], "val": 200.0, "atr_p": (1.2, 2.5), 
        "mcap": 300, "price": (500, 5000), "ma25": (-2.0, 3.0), "vol": 10, "cross": MA_CRITERIA[2], 
        "margin": (2.0, 8.0), "per": (8.0, 25.0), "ema": EMA_CRITERIA[2], "adx": (10, 20), 
...
for p, l in [("NORMAL","通常フィルター"), ("DEFENSIVE","ディフェンシブ"), ("RANGE","横ばい相場対応")]:
    ...
    if st.sidebar.button(label, key=f"side_ps_{p}", type="primary" if is_sel else "secondary"):
...
# --- 6. メインレイアウト ---
st.markdown(f"<h1 class='main-title'>FORE CASTER</h1><h3 class='sub-title'>SCREENING & BACKTEST | ver 1.78</h3>", unsafe_allow_html=True)
ticker_input = st.text_input("🎯 監視銘柄コード", st.session_state['target_tickers'])
st.session_state['target_tickers'] = ticker_input
tab_top, tab_screen, tab_bt = st.tabs(["🏠 ワンタッチ", "🔍 スクリーニング", "📈 バックテスト"])

with tab_top: # リアルタイム指標復旧
    ...
    with st.expander(f"リアルタイム指標 ({now_jst})", expanded=True):
        ...
        for n in ["日経平均", "日経先物(CME)", "ドル/円", "NYダウ30種", "原油先物(WTI)", "Gold先物(COMEX)", "VIX指数", "SOX指数"]:
            ...
            if i.get("val"):
                ...
                cards_html += f'<div class="metric-card"><div class="card-label">{n}</div><div class="card-value">{v}</div><div class="delta-badge {"plus" if i["pct"]>=0 else "minus"}">{"＋" if i["pct"]>=0 else ""}{i["pct"]:.2f}%</div></div>'
    ...
    if st.button("ワンタッチで銘柄スキャン実行", type="primary", use_container_width=True):
        ...
        if res:
            st.session_state['sc_results'] = sorted(res, key=lambda x: x['期待値'], reverse=True)[:5]
        prg.empty()
...
with tab_screen: # 🔍 スクリーニング管理 (元に戻すボタン ＆ MA/EMA選択式)
        ...
        with s_tab:
            with st.expander(f"🔍 スクリーニング設定 ({['通常', 'ディフェンシブ', '横ばい'][i]})", expanded=True):
                ...
                with c1:
                    ...
                    st.session_state['params'][pid]['atr_p'] = st.slider("期待範囲%", 0.5, 5.0, st.session_state['params'][pid]['atr_p'], key=f"v3_{pid}"); st.divider()
                ...
                with c2:
                    ...
                    st.session_state['params'][pid]['price'] = st.slider("価格(円)", 100, 10000, st.session_state['params'][pid]['price'], key=f"v5_{pid}"); st.divider()
                ...
                with c3:
                    # EMA (選択式刷新)
                    st.session_state['enabled'][pid]['ema'] = st.checkbox("EMA (9日・21日)", st.session_state['enabled'][pid]['ema'], key=f"e11_{pid}")
            ...
            st.button(f"スクリーニング実行", key=f"scr_exec_{pid}", type="primary", use_container_width=True)

with tab_bt: # 1.68 ロジック維持
    ...
    if st.button("バックテスト実行", type="primary", use_container_width=True):
        trades = []; prg = st.progress(0); end_d = datetime.now(); start_d = end_d - timedelta(days=d_back_param)
