        ...
        with s_tab:
            ...
            with st.expander(exp_t, expanded=True):
                ...
                with c1:
                    ...
                    st.selectbox("条件選択", ma_opts, index=ma_opts.index(st.session_state[f"scr_{i}_ma_opt"]), key=f"val_ma_{i}"); st.divider()
                with c2:
                    st.checkbox("**EMA (9日・21日)**", True, key=f"c_ema_{i}")
                    st.caption("直近の価格トレンド")
                    st.selectbox("EMA基準", ema_opts, index=ema_opts.index(st.session_state[f"scr_{i}_ema_opt"]), key=f"val_ema_{i}"); st.divider()
