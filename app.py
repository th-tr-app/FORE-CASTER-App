    ...
    if st.button("バックテスト実行", type="primary", use_container_width=True):
                    ...
                    for ts, row in day.iterrows():
                        if not in_pos:
                            if start_entry_bt <= ts.time() <= end_entry_bt and g_min <= gap <= g_max:
                                if (not use_vwap or row['Close'] > row['VWAP']) and (not use_ema or row['Close'] > row['EMA5']) and (not u_rsi or (row['RSI']>45 and row['RSI']>row['RSI_P'])) and (not u_macd or row['MH']>row['MHP']):
        ...
        if trades:
            ...
            for tk in t_list:
                ...
                # 平均計算 (エラー回避)
                avg_win = tw.mean() if not tw.empty else 0
                avg_loss = tl.mean() if not tl.empty else 0
                t_pf = tw.sum()/abs(tl.sum()) if not tl.empty and tl.sum()!=0 else 0
                report.append(f">>> TICKER: {tk} | {nm}")
                report.append(f"トレード数: {len(tdf)} | 勝率: {(tdf['PnL']>0).mean():.1%} | 利益平均: {avg_win:+.2%} | 損失平均: {avg_loss:+.2%} | PF: {t_pf:.2f} | 期待値: {tdf['PnL'].mean():+.2%}\n")
