...
use_vwap = st.sidebar.checkbox("**VWAP** より上でエントリー", value=True); use_ema = st.sidebar.checkbox("**EMA5** より上でエントリー", value=True)
st.sidebar.divider()
gap_min = st.sidebar.slider("寄付ダウン下限 (%)", -10.0, 0.0, -3.0, 0.05)/100; gap_max = st.sidebar.slider("寄付アップ上限 (%)", -5.0, 5.0, 1.0, 0.05)/100
st.sidebar.subheader("💰 決済ルール")
t_start = st.sidebar.number_input("トレイリング開始 (%)", 0.1, 5.0, 0.5, step=0.05)/100; s_loss = st.sidebar.number_input("損切り (%)", -5.0, -0.1, -0.7, step=0.05)/100
...
with tab_top:
    ...
    with st.expander(f"リアルタイム指標 ({now_jst})", expanded=True):
        ...
        for n, i in m_data.items():
            if i.get("val"):
                v = f"{i['val']:,.1f}" if i['val'] > 200 else f"{i['val']:,.2f}"; c = "plus" if i['pct'] >= 0 else "minus"
                cards += f'<div class="metric-card"><div class="card-label">{n}</div><div class="card-value">{v}</div><div class="delta-badge {c}">{"＋" if i["pct"]>=0 else ""}{i["pct"]:.2f}%</div></div>'
...
with tab_bt:
    ...
    if st.button("バックテスト実行", type="primary", use_container_width=True):
        ...
        for i, ticker in enumerate(tickers):
            ...
            try:
                df = yf.download(ticker, start=s_date, interval="5m", progress=False, multi_level_index=False)
                        ...
                        if not in_pos:
                            if start_entry <= ts.time() <= end_entry and gap_min <= gap <= gap_max:
                                if (not use_vwap or row['Close'] > row['VWAP']) and (not use_ema or row['Close'] > row['EMA5']):
                                    entry_p = row['Close'] * 1.0003; in_pos = True; stop_p = entry_p * (1 + s_loss); pat = get_trade_pattern(row, gap)
                        else:
                            ex_p = None
                            if row['Low'] <= stop_p: ex_p = stop_p * 0.9997
        ...
        if not res.empty:
            ...
            with bt1:
                ...
                st.markdown(f'<div class="summary-container"><div class="summary-box"><div class="summary-label">総トレード数</div><div class="summary-value">{cnt}回</div></div><div class="summary-box"><div class="summary-label">勝率</div><div class="summary-value">{wr:.1%}</div></div><div class="summary-box"><div class="summary-label">PF</div><div class="summary-value">{pf:.2f}</div></div><div class="summary-box"><div class="summary-label">期待値</div><div class="summary-value">{ev:.2%}</div></div></div>', unsafe_allow_html=True)
                report = ["=================\n BACKTEST REPORT \n================="]
                for t in tickers:
                    tdf = res[res['Ticker'] == t]
                    if tdf.empty: continue
                    t_name = TICKER_NAME_MAP.get(t, t)
                    t_wins = tdf[tdf['PnL'] > 0]; t_losses = tdf[tdf['PnL'] <= 0]
                    # 安全な期待値・平均計算
                    avg_w = t_wins['PnL'].mean() if not t_wins.empty else 0
                    avg_l = t_losses['PnL'].mean() if not t_losses.empty else 0
                    t_pf = t_wins['PnL'].sum() / abs(t_losses['PnL'].sum()) if not t_losses.empty and t_losses['PnL'].sum() != 0 else 0
            ...
            with bt2:
                # 勝ちパターン分析の表示
                st.markdown("### 🤖 パターン別成績")
                pat_df = res.groupby('Pattern')['PnL'].agg(['count', lambda x: (x>0).mean(), 'mean']).reset_index()
                pat_df.columns = ['パターン', '回数', '勝率', '平均損益']
                st.dataframe(pat_df.style.format({'勝率': '{:.1%}', '平均損益': '{:+.2%}'}), use_container_width=True, hide_index=True)
