...
st.markdown("""
    ...
    .summary-value { font-size: 28px; font-weight: 600; color: #ffffff; }
...
# --- 6. メインレイアウト ---
st.markdown(f"<h1 class='main-title'>FORE CASTER</h1><h3 class='sub-title'>SCREENING & BACKTEST | ver 1.75</h3>", unsafe_allow_html=True)
ticker_input = st.text_input("🎯 監視銘柄コード", st.session_state['target_tickers'])
st.session_state['target_tickers'] = ticker_input
tab_top, tab_screen, tab_bt = st.tabs(["🏠 ワンタッチ", "🔍 スクリーニング", "📈 バックテスト"])

# --- タブ1: ワンタッチ (Ver1.68 復旧) ---
with tab_top:
    ...
    if st.button("ワンタッチで銘柄スキャン実行", type="primary", use_container_width=True):
        sc_res = []
        prg = st.progress(0); tks = list(TICKER_NAME_MAP.keys())
        for idx, t in enumerate(tks):
            prg.progress((idx + 1) / len(tks))
            ev = run_scan_engine(t, 20, time(9,0), time(9,30), True)
            if ev and ev > 0: sc_res.append({"code": t, "name": TICKER_NAME_MAP[t], "ev": ev})
        if sc_res:
            st.session_state['sc_results'] = sorted(sc_res, key=lambda x: x['ev'], reverse=True)[:5]
        prg.empty()
    if st.session_state['sc_results']:
        st.success("🎯 期待値Top5を選出しました。")
        st.table(pd.DataFrame(st.session_state['sc_results']).rename(columns={'code':'コード','name':'銘柄名','ev':'期待値'}))

# --- タブ2: スクリーニング (最新UI) ---
with tab_screen:
    st.markdown("<br>", unsafe_allow_html=True)
    s_tabs = st.tabs(["🔍通常フィルタ", "🔍ディフェンシブ", "🔍横ばい相場"])
    for idx, s_tab in enumerate(s_tabs):
        with s_tab:
            with st.expander(f"🔍 スクリーニング設定 ({['通常', 'ディフェンシブ', '横ばい'][idx]})", expanded=True):
                c1, c2, c3 = st.columns(3)
                with c1:
                    st.checkbox("業種", True, key=f"f1_{idx}"); st.multiselect("選択", ["情報・通信", "電気機器", "銀行業"], key=f"v1_{idx}"); st.divider()
                    st.checkbox("売買代金", True, key=f"f2_{idx}"); st.number_input("億円以上", 10.0, key=f"v2_{idx}"); st.divider()
                    st.checkbox("平均値幅 (ATR%)", True, key=f"f3_{idx}"); st.slider("最小%", 0.5, 5.0, 1.5, key=f"v3_{idx}"); st.divider()
                    st.checkbox("時価総額", True, key=f"f4_{idx}"); st.number_input("億円以上", 500, key=f"v4_{idx}"); st.divider()
                with c2:
                    st.checkbox("出来高", True, key=f"f5_{idx}"); st.number_input("万株以上", 10, key=f"v5_{idx}"); st.divider()
                    st.checkbox("移動平均上抜け", False, key=f"f6_{idx}"); st.selectbox("MA種", ["5日線", "25日線", "75日線"], key=f"v6_{idx}"); st.divider()
                    st.checkbox("信用倍率", True, key=f"f7_{idx}"); st.number_input("倍率以下", 10.0, key=f"v7_{idx}"); st.divider()
                with c3:
                    st.checkbox("RSI (14日)", True, key=f"f8_{idx}"); st.slider("レンジ", 0, 100, (30, 70), key=f"v8_{idx}"); st.divider()
                    st.checkbox("ボリンジャーバンド", False, key=f"f9_{idx}"); st.slider("σ範囲", -3.0, 3.0, (1.0, 2.0), key=f"v9_{idx}"); st.divider()
                    st.checkbox("コンセンサス", True, key=f"f10_{idx}"); st.select_slider("メモリ", options=[0,1,2,3,4,5], value=3, key=f"v10_{idx}"); st.divider()
            st.button("スクリーニング実行", key=f"scr_btn_{idx}", type="primary", use_container_width=True)

# --- タブ3: バックテスト (Ver1.68 復旧) ---
with tab_bt:
    ...
    if st.button("バックテスト実行", type="primary", use_container_width=True):
        trades = []; prg = st.progress(0); end_dt = datetime.now(); start_dt = end_dt - timedelta(days=days_back_param)
        for i, ticker in enumerate(t_list):
            prg.progress((i + 1) / len(t_list))
            try:
                ...
                for d in np.unique(df.index.date):
                    ...
                    for ts, row in day.iterrows():
                        if not in_pos:
                            ...
                            if ex_p:
                                trades.append({'Ticker': ticker, 'Entry': entry_t, 'PnL': (ex_p - entry_p)/entry_p, 'In': entry_p, 'Pattern': pat, 'Gap(%)': gap_v*100, 'EntryVWAP': entry_vwap, 'Reason': rsn, 'PrevClose': pc, 'DayOpen': do})
                                in_pos = False; break
            except: continue
        prg.empty(); st.session_state['bt_results'] = pd.DataFrame(trades) if trades else None
