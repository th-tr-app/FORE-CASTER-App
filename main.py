...
st.markdown(f"<div class='header-container'><h1 class='main-title'>FORE CASTER</h1><p class='sub-title'>All-in-one Day trade manager | ver 4.85</p></div>", unsafe_allow_html=True)
        ...
        with bt_tabs[0]: # 📊 サマリー
            ...
            st.markdown(f"""
                <div class='metric-grid'>
                    ...
                    <div class='summary-box'><div class='card-label'>PF（総利益 ÷ 総損失）</div><div class='card-value'>{pf_all:.2f}</div></div>
...
with tab_strategy:
    ...
    if not t_list:
        ...
        # --- 【修正後：こちらに書き換えてください】 ---
        # 地合い乖離(m_gap)を取得。0（市場閉鎖中）なら先物(CME)の値を採用する
        m_gap = diag.get('gap_pct', 0.0)
        
        if abs(m_gap) < 0.0001:
            # m_info から先物の騰落率を取得して小数(0.0496形式)に変換
            cme_val = m_info.get("日経先物(CME)", {}).get("pct", 0.0)
            m_gap = cme_val / 100 
        # ----------------------------------------------
