...
def analyze_market_environment():
    """主要指数から今日の相場環境をプロ視点で診断する (時系列・統合版)"""
    indices = {
    ...
    if l_s <= now <= l_e:
        ...
        p_txt = "本日のトレードお疲れ様でした。明日に向け期待値の高い銘柄をランキングで精査しましょう。"
    else:
    ...
    if dev_25 > 5: balance_txt = f"【加熱】25日線乖離 +{dev_25:.1f}%。高値警戒。"; alert_lvl = "⚠️ 高値警戒"
    elif dev_25 < -5: balance_txt = f"【過売】25日線乖離 {dev_25:.1f}%。自律反発圏。"; alert_lvl = "📢 底打ち待ち"
    ...
    elif sox_p >= 0.005: u_impact = "ハイテク株への買い波及期待。主力株の底堅い展開を予想。"
...
def evaluate_screening_conditions(df, params):
    ...
    if params.get('c_gain') and not (params['gain_range'][0] <= gain <= params['gain_range'][1]): return None
    if params.get('c_v') and val_total < params.get('v_min', 0): return None
    return {"株価": int(p), "前日比": gain, "売買代金": val_total, "出来高": int(v), 
            ...
            if not in_pos:
                if params['start_t'] <= ts.time() <= params['end_t'] and params['g_min'] <= gap_v <= params['g_max']:
                    if (not params['u_vwap'] or row['Close'] > row['VWAP']) and (not params['u_ema'] or row['Close'] > row['EMA5']):
                        ...
                        sl_rec = abs(params['sl_fix']); stop_p = entry_p * (1 - sl_rec); t_high = row['High']
            else:
...
def get_one_touch_score(trades):
    ...
    return {"win_rate": win_rate, "pf": pf, "ev": ev, "score": score, "count": len(pnls), "avg_win": wins.mean() if len(wins)>0 else 0, "avg_loss": losses.mean() if len(losses)>0 else 0}
