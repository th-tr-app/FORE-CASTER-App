...
def analyze_market_environment():
    ...
    if "SOX" in data_map:
        sox_pct = (data_map["SOX"]['Close'].values.ravel()[-1] / data_map["SOX"]['Close'].values.ravel()[-2]) - 1
    ...
    if l_start <= now <= l_end:
        ...
        phase_txt = "前場のトレンドを再確認。後場はVWAP付近の攻防や高値更新に注目。"
    ...
    return {
        ...
        "alert_level": alert_lvl, "tips": tips_str
    }

# --- 3. スクリーニング・バックテストエンジン (既存維持) ---
# (evaluate_screening_conditions, fetch_daily_stats_maps, run_ticker_simulation, get_one_touch_score はお送りいただいたものをそのまま継続)
    ...
    for d in np.unique(df.index.date):
        ...
        for ts, row in day.iterrows():
            if not in_pos:
                ...
                if ex_p:
                    trades.append({'Ticker': ticker, 'PnL': (ex_p - entry_p)/entry_p, 'Pattern': get_trade_pattern(row, gap_v), 'Gap(%)': gap_v*100, 'Entry': entry_t, 'In': entry_p, 'EntryVWAP': entry_vwap})
                    in_pos = False; break
    return trades
